class GroupListsController < ApplicationController
  ...
  def create
    ...
    if @group_list.save!
      ...
      falsh.now[:error] = flash_message(:type => "uniqueness_error", :field => "list_header_id")if(!@group_list.errors[:list_header_id].nil? && @group_list.errors.on(:list_header_id).include?("has already been taken"))
    end
    ...
    def destroy
      puts "**** DEBUG destroy metho is running with param #{params[:id]}"
      @group_list = GroupList.find(params[:id].to_i)
      puts"debug---#{@group_list.to_yaml}"
      @group_list.destroy
